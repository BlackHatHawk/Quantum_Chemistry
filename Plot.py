...
def vol_Indexing_sph_angular_wave(functheta: Callable[[np.ndarray],np.ndarray],
                                    funcphi: Callable[[np.ndarray],np.ndarray],
                                    limits_theta:Tuple[np.ndarray,np.ndarray],
                                    limits_phi:Tuple[np.ndarray,np.ndarray],
                                    parts: int=1000):
    ...
    # vol = del_theta * del_phi * probabalitiy_at_that_region

    vol_index = np.zeros((1,parts**2))
    p = 1
    for i in range(parts):
        for j in range(parts):
            vol_index[0,p] = vol_index[0,p-1] + fourierTrick_spherical_angular(functheta,funcphi,(del_theta[i],del_theta[i+1]),(del_phi[j],del_phi[j+1]))
            p+=1
            
    return vol_index

def tensor_Indexing_sph_wave(funcradial: Callable[[np.ndarray],np.ndarray],
        ...
        for i in range(parts):
            for j in range(parts):
                if p ==0:
                    ...
                    prev = tensor_index[p-1] 
                tensor_index[p] = prev + fourierTrick_spherical_radial(funcradial,(del_r[h],del_r[h+1])) * fourierTrick_spherical_angular(functheta,funcphi,(del_theta[i],del_theta[i+1]),(del_phi[j],del_phi[j+1]))
                p+=1
